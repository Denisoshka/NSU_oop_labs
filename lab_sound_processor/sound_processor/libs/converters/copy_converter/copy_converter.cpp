...
namespace conv {
  void conv::CopyConverter::process(std::vector<int16_t>& MainSample,
    ...
    ++TaskInf_.CurTime;
    TaskInf_.TaskFinished = TaskInf_.CurTime >= (TaskInf_.StartTime + TaskInf_.EndTime) || TaskInf_.CurTime >= TaskInf_.InDuration;
  }
  ...
  void conv::CopyConverter::setParams(const std::vector<size_t>& kInStreams,
    ...
    acceleration_ = (kParams.size() >= kOtherParamsStart && kParams[2] != SIZE_MAX)
                          ? kParams[2]
                          : 1;
  }
