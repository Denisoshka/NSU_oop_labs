...
public:
  ...
  FlatMap(const FlatMap& otherMap)
      : FlatMap() {
    std::unique_ptr<value_type[]> tmp = std::make_shared<value_type[]>(otherMap.MaxSize_);
  ...
  void resize(size_t newSize) {
    std::unique_ptr<value_type[]> tmp = std::make_shared<value_type[]>(newSize);
...
//    alloc_traits::construct(Allocator_, std::addressof(*p), std::move_if_noexcept(toKey));
    alloc_traits::construct(Allocator_, std::addressof(*p), std::move(std::pair{toKey,value_type{}
}));
